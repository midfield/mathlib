...
lemma has_fderiv_within_at.eventually_ne {f' : E ≃L[𝕜] F}
  (h : has_fderiv_within_at f (f' : E →L[𝕜] F) s x) :
  ∀ᶠ z in 𝓝[s \ {x}] x, f z ≠ f x :=
begin
  rw [nhds_within, diff_eq, ← inf_principal, ← inf_assoc, eventually_inf_principal],
  have : (λ z, f z - f x) ~[𝓝[s] x] (λ z, f' (z - x)) := h.trans_is_O (f'.is_O_sub_rev _ _),
  simpa [not_imp_not, sub_eq_zero] using ((f'.is_O_sub_rev _ _).trans this.is_O_symm).eq_zero_imp
end

lemma has_fderiv_at.eventually_ne {f' : E ≃L[𝕜] F} (h : has_fderiv_at f (f' : E →L[𝕜] F) x) :
  ∀ᶠ z in 𝓝[{x}ᶜ] x, f z ≠ f x :=
by simpa only [compl_eq_univ_diff] using (has_fderiv_within_at_univ.2 h).eventually_ne
